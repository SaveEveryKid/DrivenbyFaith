-- =============================================================================
-- Migration 006: rotating devotional/Saturday Ready content
--
...
--
-- Backported from claude/app-completion-label-removal-pd1m1h (migration 003).
-- =============================================================================

-- ── devotional_completions: key by (user, day) instead of (user, devotional) ──

ALTER TABLE devotional_completions
  DROP CONSTRAINT IF EXISTS devotional_completions_user_id_devotional_id_key;

ALTER TABLE devotional_completions
  ADD COLUMN IF NOT EXISTS completed_date DATE NOT NULL DEFAULT CURRENT_DATE;

ALTER TABLE devotional_completions
  ADD CONSTRAINT devotional_completions_user_id_completed_date_key
    UNIQUE (user_id, completed_date);

CREATE INDEX IF NOT EXISTS idx_devotional_completions_completed_date
  ON devotional_completions(completed_date);

-- ── saturday_ready_responses: key by (user, week) instead of (user, entry) ──

ALTER TABLE saturday_ready_responses
  DROP CONSTRAINT IF EXISTS saturday_ready_responses_user_id_saturday_ready_id_key;

ALTER TABLE saturday_ready_responses
  ADD COLUMN IF NOT EXISTS response_date DATE NOT NULL DEFAULT CURRENT_DATE;

ALTER TABLE saturday_ready_responses
  ADD CONSTRAINT saturday_ready_responses_user_id_response_date_key
    UNIQUE (user_id, response_date);

CREATE INDEX IF NOT EXISTS idx_srr_response_date
  ON saturday_ready_responses(response_date);
